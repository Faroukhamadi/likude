...
func (c *CommunityQuery) Paginate(
	...
	conn.Edges = make([]*CommunityEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		conn.Edges[i] = &CommunityEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
func (r *ReplyQuery) Paginate(
	...
	conn.Edges = make([]*ReplyEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		conn.Edges[i] = &ReplyEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
// SubreplyEdge is the edge representation of Subreply.
type SubreplyEdge struct {
	Node   *Subreply `json:"node"`
	Cursor Cursor    `json:"cursor"`
}

// SubreplyConnection is the connection containing edges to Subreply.
type SubreplyConnection struct {
	Edges      []*SubreplyEdge `json:"edges"`
	PageInfo   PageInfo        `json:"pageInfo"`
	TotalCount int             `json:"totalCount"`
}

// SubreplyPaginateOption enables pagination customization.
type SubreplyPaginateOption func(*subreplyPager) error

// WithSubreplyOrder configures pagination ordering.
func WithSubreplyOrder(order *SubreplyOrder) SubreplyPaginateOption {
	if order == nil {
		order = DefaultSubreplyOrder
	}
	o := *order
	return func(pager *subreplyPager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultSubreplyOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithSubreplyFilter configures pagination filter.
func WithSubreplyFilter(filter func(*SubreplyQuery) (*SubreplyQuery, error)) SubreplyPaginateOption {
	return func(pager *subreplyPager) error {
		if filter == nil {
			return errors.New("SubreplyQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type subreplyPager struct {
	order  *SubreplyOrder
	filter func(*SubreplyQuery) (*SubreplyQuery, error)
}

func newSubreplyPager(opts []SubreplyPaginateOption) (*subreplyPager, error) {
	pager := &subreplyPager{}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
	...
	if pager.order == nil {
		pager.order = DefaultSubreplyOrder
	}
	return pager, nil
}

func (p *subreplyPager) applyFilter(query *SubreplyQuery) (*SubreplyQuery, error) {
	if p.filter != nil {
		return p.filter(query)
	}
	return query, nil
...
func (p *subreplyPager) toCursor(s *Subreply) Cursor {
	return p.order.Field.toCursor(s)
}

func (p *subreplyPager) applyCursors(query *SubreplyQuery, after, before *Cursor) *SubreplyQuery {
	for _, predicate := range cursorsToPredicates(
		p.order.Direction, after, before,
		p.order.Field.field, DefaultSubreplyOrder.Field.field,
	) {
		query = query.Where(predicate)
	}
	return query
...
func (p *subreplyPager) applyOrder(query *SubreplyQuery, reverse bool) *SubreplyQuery {
	direction := p.order.Direction
	if reverse {
		direction = direction.reverse()
	}
	query = query.Order(direction.orderFunc(p.order.Field.field))
	if p.order.Field != DefaultSubreplyOrder.Field {
		query = query.Order(direction.orderFunc(DefaultSubreplyOrder.Field.field))
	}
	return query
}

// Paginate executes the query and returns a relay based cursor connection to Subreply.
func (s *SubreplyQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...SubreplyPaginateOption,
) (*SubreplyConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newSubreplyPager(opts)
	if err != nil {
		return nil, err
	}
	...
	conn := &SubreplyConnection{Edges: []*SubreplyEdge{}}
	if !hasCollectedField(ctx, edgesField) || first != nil && *first == 0 || last != nil && *last == 0 {
		if hasCollectedField(ctx, totalCountField) ||
			hasCollectedField(ctx, pageInfoField) {
	...
	var nodeAt func(int) *Subreply
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *Subreply {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *Subreply {
	...
	conn.Edges = make([]*SubreplyEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		conn.Edges[i] = &SubreplyEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
// SubreplyOrderField defines the ordering field of Subreply.
type SubreplyOrderField struct {
	field    string
	toCursor func(*Subreply) Cursor
}

// SubreplyOrder defines the ordering of Subreply.
type SubreplyOrder struct {
	Direction OrderDirection      `json:"direction"`
	Field     *SubreplyOrderField `json:"field"`
}

// DefaultSubreplyOrder is the default ordering of Subreply.
var DefaultSubreplyOrder = &SubreplyOrder{
	Direction: OrderDirectionAsc,
	Field: &SubreplyOrderField{
		field: subreply.FieldID,
		toCursor: func(s *Subreply) Cursor {
			return Cursor{ID: s.ID}
		},
	},
}

// ToEdge converts Subreply into SubreplyEdge.
func (s *Subreply) ToEdge(order *SubreplyOrder) *SubreplyEdge {
	if order == nil {
		order = DefaultSubreplyOrder
	}
	return &SubreplyEdge{
...
func (t *TopicQuery) Paginate(
	...
	conn.Edges = make([]*TopicEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		conn.Edges[i] = &TopicEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
func (tr *TopicRelatedQuery) Paginate(
	...
	conn.Edges = make([]*TopicRelatedEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		conn.Edges[i] = &TopicRelatedEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
