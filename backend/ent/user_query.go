...
func (uq *UserQuery) QueryCommunities() *CommunityQuery {
	query := &CommunityQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withCommunities: uq.withCommunities.Clone(),
...
func (uq *UserQuery) WithCommunities(opts ...func(*CommunityQuery)) *UserQuery {
	query := &CommunityQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withCommunities = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withCommunities; query != nil {
		...
		for i, node := range nodes {
			edgeids[i] = node.ID
			byid[node.ID] = node
			node.Edges.Communities = []*Community{}
		}
		query.Where(func(s *sql.Selector) {
			joinT := sql.Table(user.CommunitiesTable)
			s.Join(joinT).On(s.C(community.FieldID), joinT.C(user.CommunitiesPrimaryKey[0]))
			s.Where(sql.InValues(joinT.C(user.CommunitiesPrimaryKey[1]), edgeids...))
			columns := s.SelectedColumns()
			s.Select(joinT.C(user.CommunitiesPrimaryKey[1]))
		...
		for _, n := range neighbors {
			nodes, ok := nids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "communities" node returned %v`, n.ID)
			}
			for kn := range nodes {
				kn.Edges.Communities = append(kn.Edges.Communities, n)
			}
		}
	}
