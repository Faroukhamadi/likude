...
// The TopicRelatedFunc type is an adapter to allow the use of ordinary
// function as TopicRelated mutator.
type TopicRelatedFunc func(context.Context, *ent.TopicRelatedMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TopicRelatedFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.TopicRelatedMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TopicRelatedMutation", m)
	}
	return f(ctx, mv)
}
